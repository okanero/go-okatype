...
// Type represents the magic constant value at the beginning of the serialization of an Okanero Content
// (i.e., a serialization of okatype_content.Type).
//
// This is used in the okatype_content.Type struct.
//
// To put some of this into content,....
//
// (And first we'll talk about file formats, even though okatype_contentmagic.Type isn't used in a file format per se.
...
//
// Yet another strategy (and the strategy we are making use of with okatype_contentmagic.Type) is the usage
// of a "magic".
//
...
//
// The okatype_contentmagic.Type use to create a "magic" / "magic numbers" / file signature" for okatype_content.Type.
type Type struct{}

var (
	// The value of `magic` is the magic value (sequence of bytes) at the beginning of a
	// serialization of an Okanero Content (i.e., a serialization of okatype_content.Type).
	//
	// (A "serialization" is how you would turn an okatype_content.Type into a []byte,
	...
	magic [8]byte = [8]byte{7,'c','o','n','t','e','n','t'}
)
