...
func TestMagicWriterTo(t *testing.T) {
	...
	var x io.WriterTo = value // THIS IS WHAT ACTUALLY MATTERS.
...
func TestMagicWriteTo(t *testing.T) {
...

	{
		...
		const expected1 byte = 'c'
		const expected2 byte = 'o'
		const expected3 byte = 'n'
		const expected4 byte = 't'
		const expected5 byte = 'e'
		const expected6 byte = 'n'
		const expected7 byte = 't'
