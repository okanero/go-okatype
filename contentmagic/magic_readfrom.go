...
// ReadFrom makes okatype_contentmagic.Type fit the io.ReaderFrom interface.
func (receiver *Type) ReadFrom(r io.Reader) (int64, error) {
	...
	if nil != err {
		...
		return n64, fmt.Errorf("Problem reading in magic for Okanero Content: (%T) %q", err, err)
	}
